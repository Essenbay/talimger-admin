...
import 'package:increatorkz_admin/forms/lesson_form.dart';
...
import 'package:increatorkz_admin/utils/article_preview.dart';
import 'package:increatorkz_admin/utils/quiz_preview.dart';
...
class LessonsMixin {
  ...
  Widget _lessonList(
    ...
    return ReorderableListView.builder(
      ...
      onReorder: (oldIndex, newIndex) => _onReorder(
          context, oldIndex, newIndex, snapshot, courseDocId, sectionId, ref),
      ...
      itemBuilder: (BuildContext listContext, int index) {
        ...
        final List<Lesson> lessons =
            snapshot.docs.map((e) => Lesson.fromFiresore(e)).toList();
        final Lesson lesson = lessons[index];
        return _buildListItem(
            context, lesson, courseDocId, sectionId, index, ref, isPreview);
      },
    );
  }
          ...
          children: [
            ...
            Expanded(
              child: ListTile(
                ...
                trailing: Wrap(
                  ...
                  children: [
                    ...
                    Visibility(
                      ...
                      child: Padding(
                        ...
                        child: CustomButtons.circleButton(
                          context,
                          onPressed: () =>
                              _onEdit(context, courseDocId, sectionId, lesson),
                          icon: Icons.edit,
                        ),
                      ),
                    ),
                    Visibility(
                      ...
                      child: CustomButtons.circleButton(
                        context,
                        onPressed: () => _onDeleteLesson(
                            context, courseDocId, sectionId, lesson, ref),
                        icon: Icons.delete,
                      ),
                    ),
                  ],
                ...
                subtitle:
                    Text(lessonTypes(context)[lesson.contentType].toString()),
              ),
            ),
          ],
        ),
  ...
  void _onLessonPreview(context, Lesson lesson) {
    if (lesson.contentType == lessonTypes(context).keys.elementAt(0)) {
      //video
      AppService().openLink(context, lesson.videoUrl.toString());
    }
    if (lesson.contentType == lessonTypes(context).keys.elementAt(1)) {
      //article
      CustomDialogs.openResponsiveDialog(context,
          widget: ArticlePreview(lesson: lesson));
    }
    if (lesson.contentType == lessonTypes(context).keys.elementAt(2)) {
      //quiz
      CustomDialogs.openResponsiveDialog(context,
          widget: QuizPreview(lesson: lesson));
    }
  }
  ...
  void _onEdit(context, String courseId, String sectionId, Lesson lesson) {
    CustomDialogs.openResponsiveDialog(
      ...
      widget: LessonForm(
          courseDocId: courseId, sectionDocId: sectionId, lesson: lesson),
    );
  }

  void _onDeleteLesson(BuildContext context, String courseDocId,
      String sectionId, Lesson lesson, WidgetRef ref) async {
    ...
    if (UserMixin.hasAccess(ref.read(userDataProvider))) {
      CustomDialogs.openActionDialog(
        ...
        message:
            'Warning: All of the contents releated to this lesson will be deleted and this can not be undone!',
        onAction: () async {
          deleteBtnCtlr.start();
          await FirebaseService()
              .deleteLesson(courseDocId, sectionId, lesson.id);
          await FirebaseService()
              .updateLessonCountInCourse(courseDocId, count: (-1));
  ...
  void _onReorder(
    ...
    if (UserMixin.hasAccess(ref.read(userDataProvider))) {
      ...
      await FirebaseService()
          .updateLessonsOrder(lessons, courseDocId, sectionId);
    } else {
      openTestingToast(context);
    }
  }
