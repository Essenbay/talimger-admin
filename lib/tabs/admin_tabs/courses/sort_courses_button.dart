  ...
  Widget build(BuildContext context) {
    ...
    return PopupMenuButton(
            ...
            Visibility(
              ...
              child: Row(
                ...
                children: [
                  ...
                  Text(
                    '${context.localized.sort_by} - $sortText',
      ...
      itemBuilder: (BuildContext context) {
        return sortByCourse(context).entries.map((e) {
      ...
      onSelected: (dynamic value) {
        ref
            .read(sortByCourseTextProvider.notifier)
            .update((state) => sortByCourse(context)[value].toString());

        if (value == 'all') {
          final newQuery = colRef.orderBy('created_at', descending: true);
          ref.read(courseQueryprovider.notifier).update((state) => newQuery);
        }
        if (courseStatus(context).containsKey(value)) {
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return ListTile(
                      ...
                      onTap: () {
                        ref
                            .read(sortByCourseTextProvider.notifier)
                            .update((state) => authors[index].name);
                        final newQuery = colRef.where('author.id',
                            isEqualTo: authors[index].id);
                        ref
                            .read(courseQueryprovider.notifier)
                            .update((state) => newQuery);
                  ...
                  itemBuilder: (BuildContext context, int index) {
                    return ListTile(
                      ...
                      onTap: () {
                        ref
                            .read(sortByCourseTextProvider.notifier)
                            .update((state) => cList[index].name);
                        final newQuery =
                            colRef.where('cat_id', isEqualTo: cList[index].id);
                        ref
                            .read(courseQueryprovider.notifier)
                            .update((state) => newQuery);
                        Navigator.pop(context);
