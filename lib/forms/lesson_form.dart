...
import 'package:increatorkz_admin/components/questions.dart';
import 'package:increatorkz_admin/configs/constants.dart';
import 'package:increatorkz_admin/extentions/context.dart';
import 'package:increatorkz_admin/forms/quiz_form.dart';
...
class _LessonFormState extends ConsumerState<LessonForm> with TextFields {
  ...
  late String _contentType = lessonTypes(context).keys.elementAt(0);
  ...
  void initState() {
    if (widget.lesson != null) {
      ...
      Future.microtask(() {
        ...
        ref
            .read(questionListProvider.notifier)
  ...
  Future<bool> _contentTypeValidation() async {
    ...
    if (_contentType == lessonTypes(context).keys.elementAt(1)) {
      final String description = await descriptionCtlr.getText();
      if (description.isEmpty) {
        validated = false;
        if (mounted) {
          openFailureToast(context, context.localized.article_empty);
        }
      } else {
        validated = true;
      }
    } else if (_contentType == lessonTypes(context).keys.elementAt(2)) {
      final questions = ref.read(questionListProvider);
      if (questions.isEmpty) {
        openFailureToast(context, context.localized.article_question);
        validated = false;
      } else {
        validated = true;
      }
  ...
  Future _handleUpload() async {
    final String? description =
        _contentType == lessonTypes(context).keys.elementAt(1)
            ? await descriptionCtlr.getText()
            : null;
    final Lesson lesson = _lessonData(description);
    await FirebaseService()
        .saveLesson(widget.courseDocId, widget.sectionDocId, lesson);
    if (widget.lesson == null) {
      await FirebaseService()
          .updateLessonCountInCourse(widget.courseDocId, count: 1);
    }
  }
          ...
          child: Column(
            ...
            children: [
              ...
              Padding(
                ...
                child: RadioOptions(
                  contentType: _contentType,
                  options: lessonTypes(context),
                  icon: LineIcons.list,
                  title: context.localized.lesson_type,
                  onChanged: (value) {
                    setState(() {
                      _contentType = value;
                    });
              ...
              Visibility(
                visible: _contentType == lessonTypes(context).keys.elementAt(0),
                child: Padding(
                  padding: const EdgeInsets.symmetric(vertical: 30),
                  child: buildTextField(
                    context,
                    controller: videoCtlr,
                    hint: context.localized.video,
                    title: '${context.localized.video} *',
                    hasImageUpload: false,
              ...
              Visibility(
                visible: _contentType == lessonTypes(context).keys.elementAt(1),
                child: Padding(
                    padding: const EdgeInsets.symmetric(vertical: 30),
                    child: CustomHtmlEditor(
                      ...
                      title: '${context.localized.article} *',
                    )),
              ),
              Visibility(
                visible: _contentType == lessonTypes(context).keys.elementAt(2),
                child: const Padding(
